...
impl State {
    ...
    pub fn init_accesskit(
        ...
        // TODO: accesskit_winit 0.29.1 is incompatible with winit 0.31.
        // Update accesskit_winit to a version that supports winit 0.31.
        log::warn!("accesskit integration is not yet supported with winit 0.31");
    }
    ...
    pub fn on_window_event(
        ...
        // TODO: accesskit_winit 0.29.1 is incompatible with winit 0.31 event types.
        // Re-enable when accesskit_winit supports winit 0.31.
        // #[cfg(feature = "accesskit")]
        ...
        match event {
            ...
            WindowEvent::PointerButton {
                ...
                // Update pointer position from the button event
                self.on_cursor_moved(window, *position);
                match button {
                    winit::event::ButtonSource::Mouse(mouse_button) => {
                        self.on_mouse_button_input(*state, *mouse_button);
                    }
                    winit::event::ButtonSource::Touch { finger_id, force } => {
                        self.on_touch_button(window, *finger_id, *state, *position, *force);
                    }
                    _ => {}
                }
                EventResponse {
            ...
            WindowEvent::PointerMoved {
                ...
                self.on_cursor_moved(window, *position);
                // Handle touch events delivered via PointerMoved
                if let winit::event::PointerSource::Touch { finger_id, force } = source {
                    self.on_touch_moved(window, *finger_id, *position, *force);
                }
                EventResponse {
                    repaint: true,
    ...
    fn on_touch_moved(
        ...
        self.egui_input.events.push(egui::Event::Touch {
            device_id: egui::TouchDeviceId(egui::epaint::util::hash(finger_id)),
            id: egui::TouchId::from(finger_id.into_raw() as u64),
            phase: egui::TouchPhase::Move,
            pos,
            force: force_value,
        });
    ...
    fn on_touch_button(
        ...
        // Emulate mouse button for touch
        if pressed {
    ...
    fn set_cursor_icon(&mut self, window: &(impl Window + ?Sized), cursor_icon: egui::CursorIcon) {
        ...
        if is_pointer_in_window {
            ...
            if let Some(winit_cursor_icon) = translate_cursor(cursor_icon) {
                ...
                window.set_cursor(winit::cursor::Cursor::Icon(winit_cursor_icon));
            } else {
...
pub fn update_viewport_info(
    ...
    let monitor_size = {
        ...
        {
            let size = video_mode.size().to_logical::<f32>(pixels_per_point.into());
...
fn translate_cursor(cursor_icon: egui::CursorIcon) -> Option<WinitCursorIcon> {
    match cursor_icon {
        ...
        egui::CursorIcon::PointingHand => Some(WinitCursorIcon::Pointer),
    ...
    match command {
        ...
        ViewportCommand::OuterPosition(pos) => {
            window.set_outer_position(
                PhysicalPosition::new(pixels_per_point * pos.x, pixels_per_point * pos.y).into(),
            );
        }
        ViewportCommand::MinInnerSize(s) => {
            window.set_min_surface_size(
                (s.is_finite() && s != Vec2::ZERO)
                    .then_some(PhysicalSize::new(
                        pixels_per_point * s.x,
                        pixels_per_point * s.y,
                    ))
                    .map(Into::into),
            );
        }
        ...
        ViewportCommand::Fullscreen(v) => {
            window.set_fullscreen(v.then_some(winit::monitor::Fullscreen::Borderless(None)));
        }
        ...
        ViewportCommand::CursorPosition(pos) => {
            if let Err(err) = window.set_cursor_position(
                PhysicalPosition::new(pixels_per_point * pos.x, pixels_per_point * pos.y).into(),
            ) {
                log::warn!("{command:?}: {err}");
            }
...
pub fn create_window(
    ...
    apply_viewport_builder_to_window(egui_ctx, window.as_ref(), viewport_builder);
    Ok(window)
}
...
pub fn create_winit_window_attributes(
    ...
    let mut window_attributes = winit::window::WindowAttributes::default()
        ...
        .with_fullscreen(
            fullscreen.and_then(|e| e.then_some(winit::monitor::Fullscreen::Borderless(None))),
        )
    ...
    if let Some(app_id) = _app_id {
        let platform_attributes =
            winit::platform::wayland::WindowAttributesWayland::default().with_name(app_id, "");
        window_attributes =
            window_attributes.with_platform_attributes(Box::new(platform_attributes));
    }

    #[cfg(all(feature = "x11", target_os = "linux"))]
    {
        let mut platform_attributes = winit::platform::x11::WindowAttributesX11::default();

        if let Some(window_type) = _window_type {
            use winit::platform::x11::WindowType;
            platform_attributes =
                platform_attributes.with_x11_window_type(vec![match window_type {
        ...
        if let Some(override_redirect) = _override_redirect {
            platform_attributes = platform_attributes.with_override_redirect(override_redirect);
        }
        window_attributes =
            window_attributes.with_platform_attributes(Box::new(platform_attributes));
    }
    ...
    {
        // In winit 0.31, with_drag_and_drop and with_skip_taskbar are removed.
        // Drag and drop is always enabled. Taskbar visibility is not configurable via WindowAttributes.
        _ = _drag_and_drop;
        _ = _taskbar;
    }
...
fn to_winit_icon(icon: &egui::IconData) -> Option<winit::icon::Icon> {
    if icon.is_empty() {
        None
    } else {
        profiling::function_scope!();
        match winit::icon::RgbaIcon::new(icon.rgba.clone(), icon.width, icon.height) {
            Ok(rgba_icon) => Some(winit::icon::Icon::from(rgba_icon)),
            Err(err) => {
...
pub fn short_window_event_description(event: &winit::event::WindowEvent) -> &'static str {
    ...
    match event {
        ...
        WindowEvent::SurfaceResized { .. } => "WindowEvent::SurfaceResized",
